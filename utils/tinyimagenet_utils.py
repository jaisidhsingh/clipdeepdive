...
def id2class_mapping():
	save_path = "../data/tinyimagenet/id2class_mapping.pt"	
	if os.path.exists(save_path):
		return save_path

	else:
		anno_file = "../data/tinyimagenet/words.txt"
		mapping = {}
...
def train_image_class():
	train_data_folder = "../data/tinyimagenet/train"
...
def val_image_class():
	...
	if os.path.exists(save_path):
		...
		mapping_path = "../data/tinyimagenet/val_image_ids.pt"	
		mapping = torch.load(mapping_path)
		dataset_mapping = {}
...
def basic_prompt_features(classes):
	save_path = "../data/tinyimagenet/basic_class_prompt_features.pt"
	if os.path.exists(save_path):
		return save_path
...
def extended_prompt_features(classes, attributes):
	save_path = "../data/tinyimagenet/extended_class_prompt_features.pt"
	if os.path.exists(save_path):
		return save_path
...
def get_class_prompt_features(classes, mode="basic"):
	if mode == "basic":
		...
		attributes = torch.load("../data/imagenet/imagenet_class_attributes.pt")
